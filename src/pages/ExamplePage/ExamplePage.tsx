...
} from '../../store/ProjectApi/projectApi';
...
import {
  ...
  useGetGoodsQuery,
} from '../../store/ProjectApi/goodsApi';
...
export const ExamplePage = () => {
  ...
  const [
    fetchRepos,
    { isLoading: isReposLoading, isError: getReposError, data: repos },
  ] = useLazyGetUserReposQuery();
  ...
  return (
    <div className={classes['examplePage']} data-testid="examplePage">
      ...
      <div>
        ...
        {repos?.map((repo) => (
          <li key={repo.id} onClick={(e) => addFavourite(e, repo.url)}>
            {repo.url}
          </li>
        ))}
      </div>
      ...
      <ul>
        {favourites.map((fav) => (
          <li key={fav} onClick={() => removeFavourite(fav)}>
            {fav}
          </li>
        ))}
      </ul>
      ...
      <ul>
        {goods?.map((good) => (
          <li key={good.id} onClick={() => handleDeleteProduct(good.id)}>
            {good.title}
          </li>
        ))}
      </ul>
